...
def generate_example(id_fund):
    ...
    ms page.
    '''
    general_page = requests.get('https://www.morningstar.es/es/funds/snapshot/snapshot.aspx?id=' + id_fund)
    soup = BeautifulSoup(general_page.content, 'html.parser')
    with open(f'test_pages/{id_fund}_general.html', 'w') as f:
        print(soup.prettify, file=f) 
    
    rating_risk_page = requests.get(f'https://www.morningstar.es/es/funds/snapshot/snapshot.aspx?id={id_fund}&tab=2')
